...
export interface PuzzleViewProps {
  /** Endpoint that returns a puzzle payload (today's, or one by id). */
  endpoint: string;
  /** Where the back arrow / "back to menu" link points. Defaults to home. */
  backHref?: string;
  /** True when this is an archived (past) puzzle rather than today's. Drives
   *  the share copy, the archive-solve achievement, and the not-found text. */
  isArchive?: boolean;
}

/** The full daily-puzzle experience, generic over its source so both the
 *  daily puzzle (/puzzle) and an archived one (/puzzle/[id]) render through
 *  the same component. */
export default function PuzzleView({ endpoint, backHref = '/', isArchive = false }: PuzzleViewProps) {
  const { user, profile, loading: userLoading } = useUser();
  const { theme, isRTL, t, localeId } = useSettings();
  const [load, setLoad] = useState<LoadState>({ kind: 'loading' });

  // Fetch the puzzle on mount.
  useEffect(() => {
    let cancelled = false;
    fetch(endpoint)
      .then(async res => {
        ...
        if (res.status === 404) {
          setLoad(isArchive
            ? { kind: 'error', message: t('puzzle.archive.notFound') }
            : { kind: 'no-puzzle' });
          return;
        }
...
function PuzzleSession({ puzzle, locale, backHref, isArchive }: PuzzleSessionProps) {
  ...
  useEffect(() => {
    ...
    const fresh = unlock(earnedFromPuzzle(wrongCount, isArchive));
    if (fresh.length) setNewAchievements(fresh);
  }, [status, wrongCount, isArchive]);
      ...
      >
        ...
        {status === 'solved' && (
          <SolvedCard
            ...
            shareHref={isArchive ? `/puzzle/${puzzle.id}` : '/puzzle'}
            isArchive={isArchive}
          />
        )}
...
function SolvedCard({
  ...
  // shared URL points at the puzzle that was solved — today's puzzle
  // for the daily, or the specific archived puzzle's permalink.
  async function handleShare() {
    const origin = typeof window !== 'undefined' ? window.location.origin : 'https://zaeer-imenet.vercel.app';
    const url = `${origin}${shareHref}`;
    const what = isArchive ? 'a Zaeer Imenet puzzle' : `today's Zaeer Imenet puzzle`;
    const headline = wrongCount === 0
      ? `🏆 Solved ${what} on the first try!`
      : `🏆 Solved ${what} (${wrongCount + 1} tries).`;
    const text = `${headline} Try it: ${url}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Zaeer Imenet Puzzle', text, url });
        return;
      }
    } catch {
